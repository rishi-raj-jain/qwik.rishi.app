...
export const addCopyToCode = (text: string) => {
...
const highlightHref = (text: string) => {
  const $ = load(`<html><head></head><body>${text}</body></html>`)
  $('a').each((_, i) => {
...
export default async function MD_TO_HTML(markdown: string, highlightHrefs = false) {
  let result: VFile | string | null = await unified().use(remarkParse).use(prism).use(remarkRehype).use(rehypeStringify).process(markdown)
  result = addCopyToCode(result.toString())
  if (highlightHrefs && result) {
